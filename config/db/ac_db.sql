/* NIWA-MPI ACOUSTIC DATABASE GENERATION SCRIPT, version 0.2.1
...
(	
	...
	UNIQUE (ship_type) ON CONFLICT IGNORE
);
COMMENT ON TABLE t_ship_type is 'Controlled vocabulary for ship_type attribute in t_ship.';
...
(	
	...
	UNIQUE (deployment_type) ON CONFLICT IGNORE
);
...
* 1 calibration refers to one particular setup.
...
(
	...
	UNIQUE (transducer_beam_type) ON CONFLICT IGNORE
);
...
(
	...
	UNIQUE (platform_type) ON CONFLICT IGNORE
	);
...
(
	...
	UNIQUE (transducer_location_type) ON CONFLICT IGNORE
);
...
(
	...
	UNIQUE (transducer_orientation_type) ON CONFLICT IGNORE
);
...
(
	...
	UNIQUE (setup_platform_type_key,setup_transceiver_key,setup_transducer_key,setup_parameters_key,setup_transducer_location_x,setup_transducer_location_y,setup_transducer_location_z,setup_transducer_depth,setup_transducer_orientation_type_key,setup_transducer_orientation_vx,setup_transducer_orientation_vy,setup_transducer_orientation_vz) ON CONFLICT IGNORE --ui_t_setup
);
COMMENT ON TABLE t_setup is 'Individual combinations of 1 transducer and 1 transceiver, on 1 platform/location, with 1 orientation, and 1 set of acquisition parameters, to which one channel in an acoustic file can be uniquely linked.';
...
(
	...
	calibration_acquisition_method_type_key	INTEGER,		
	...
	calibration_setup_key			INTEGER, 	-- setup to which the calibration refers to (t_setup table)
	calibration_frequency			FLOAT,		-- frequency at which calibration parameters apply to for this setup (Hz)
	calibration_gain				FLOAT,		-- on-axis gain for this setup (db)
	calibration_sacorrect			FLOAT,		-- sa correction as applied on Simrad systems (db)
	calibration_phi_athwart			FLOAT,		-- estimated athwart beam angle (angular degrees)
	calibration_phi_along			FLOAT,		-- estimated along beam angle (angular degrees)
	calibration_phi_athwart_offset	FLOAT,		-- estimated athwart beam angle offset (angular degrees)
	calibration_phi_along_offset	FLOAT,		-- estimated along beam angle offset (angular degrees)
	calibration_psi					FLOAT,		-- estimated equivalent bean angle (db)
	...
	FOREIGN KEY (calibration_setup_key) REFERENCES t_setup(setup_pkey),
	...
	UNIQUE (calibration_date,calibration_setup_key) ON CONFLICT REPLACE
);
