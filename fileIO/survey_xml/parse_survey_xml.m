...
function cal = get_cal_node(node,freq,eqa,cid)
...
for iu = 1:length(trans_nodes)
    ...
    if isfield(trans_curr,'number')
       if ischar(trans_curr.number)
           trans_curr.number=str2double(strsplit(trans_curr.number,';'));
           trans_curr.number(isnan(trans_curr.number))=[];
       end
    end
    
    bott_curr = get_childs(trans_nodes(iu),'bottom');
    reg_curr = get_childs(trans_nodes(iu),'region');
    cell_curr = get_childs(trans_nodes(iu),'cells');
    
    trans_curr.Bottom = get_node_att(bott_curr);
    trans_curr.Regions = cell(1,length(reg_curr));
    trans_curr.Cells = cell(1,length(cell_curr));
    
    for iiu = 1:length(reg_curr)
        trans_curr.Regions{iiu} = get_node_att(reg_curr(iiu));
    end
    
    for iiu = 1:length(cell_curr)
        trans_curr.Cells{iiu} = get_node_att(cell_curr(iiu));
    end
