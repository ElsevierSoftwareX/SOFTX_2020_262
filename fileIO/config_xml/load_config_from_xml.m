function [app_path,curr_disp,algo_vec,algo_alt]=load_config_from_xml(bool_app,bool_disp,bool_algos)
...
if bool_app
    ...
    try
        if isfile(path_config_file)
            app_path=read_config_path_xml(path_config_file);
        else
            disp('No XML path config file. Creating a standard one');
...
if bool_disp
    try
        if isfile(display_config_file)
            curr_disp=read_config_display_xml(display_config_file);
            curr_disp.Grid_x=[0 0 0];
            curr_disp.Grid_y=0;
        else
            disp('No XML display config file. Creating a standard one');
