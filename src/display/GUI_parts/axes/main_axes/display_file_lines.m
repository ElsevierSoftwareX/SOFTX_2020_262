...
for ifile=1:length(idx_change_file)
    %plot(axes_panel_comp.echo_obj.main_ax,xdata(idx_change_file(ifile)).*ones(size(ydata))+1/2,ydata,'color',col_lab,'Tag','file_id');
    xline(axes_panel_comp.echo_obj.main_ax,xdata(idx_change_file(ifile))+1/2,'color',col_lab,'Tag','file_id','Label',layer.Filename{ifile},'Interpreter','none');
end
