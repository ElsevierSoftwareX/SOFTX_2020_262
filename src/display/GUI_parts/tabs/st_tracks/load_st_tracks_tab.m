...
st_tracks_tab_comp.ax_pos=axes('Parent',st_tracks_tab_comp.st_tracks_tab,'Units','normalized',...
    'OuterPosition',[0 0 1/3 1],'visible','on','box','on','tag','st_comp','nextplot','add','Yaxislocation','origin','Xaxislocation','origin');
...
menu = uicontextmenu(ifig);
uimenu(menu,'Label','Display compensated TS from ST','Tag','st_comp', 'Checked', 'on','Callback',@change_disp_comp);
uimenu(menu,'Label','Display uncompensated TS from ST','Tag','st_uncomp', 'Checked', 'off','Callback',@change_disp_comp);
uimenu(menu,'Label','Display compensated TS from tracks','Tag','tracks_comp', 'Checked', 'off','Callback',@change_disp_comp);
uimenu(menu,'Label','Display uncompensated TS from tracks','Tag','tracks_uncomp', 'Checked', 'off','Callback',@change_disp_comp);

st_tracks_tab_comp.ax_pos.UIContextMenu =  menu;
...
st_tracks_tab_comp.ax_pos.Tag = src.Tag;
 update_st_tracks_tab(main_figure,'st',1,'histo',0);
