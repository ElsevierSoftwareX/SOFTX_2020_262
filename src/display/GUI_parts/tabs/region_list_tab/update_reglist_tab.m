...
reglist_tab_comp.table.ColumnWidth='auto';
...
if ~isempty(reglist_tab_comp.table.Data)&&~force_repop
    ...
    if numel({regions(:).Tag})==numel(reglist_tab_comp.table.Data(:,3))
        idx_same=cellfun(@(x) find(strcmpi(x,reglist_tab_comp.table.Data(:,10))),{regions(:).Unique_ID});
        idx_reg_to_update=find(~strcmpi({regions(:).Tag},reglist_tab_comp.table.Data(idx_same,3)'));
