...
for i_cell=1:length(Filename_cell)
    ...
    for ui=1:2
        ...
        dd_t=swapbytes(typecast(dd(id),'uint16')); 
        
        id_flag_tmp=2*find(dd_t==flag_start)+ui-1;
        id_flag=union(id_flag,id_flag_tmp);
        id_serial_tmp=2*find(dd_t==serial)+ui-1;
        id_serial=union(id_serial,id_serial_tmp);
        
    end
    
    id_flag=intersect(id_flag,id_serial-4);
    
    pos_id=id_flag-2;
    
    data=init_data(numel(id_flag));
