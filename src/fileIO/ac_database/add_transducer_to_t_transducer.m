...
%struct_in=rmfield(struct_in,{'transducer_comments' 'transducer_frequency_lower' 'transducer_frequency_upper'});
ukeys={'transducer_model','transducer_serial','transducer_frequency_nominal'};
for ik=1:numel(ukeys)
    struct_in_unique.(ukeys{ik})=struct_in.(ukeys{ik});
end
[~,transducer_pkey]=get_cols_from_table(ac_db_filename,'t_transducer','input_struct',struct_in_unique,'output_cols',{'transducer_pkey'});