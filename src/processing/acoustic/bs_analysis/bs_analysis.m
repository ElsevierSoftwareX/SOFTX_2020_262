...
addParameter(p, 'PhiStdThr',  30/180*pi,         @isnumeric);
addParameter(p, 'AttCal',     [0 0 0],      @isnumeric);
...
idy_along_add=along_est.delta(idx_nan) < phi_std_thr;
true_idx_bottom(idx_nan(idy_along_add)) = along_est.idx(idx_nan(idy_along_add));
...
Yaw_r = att_recept.Heave(:)'+yaw_cal;
...
plot(across_est.slope/pi*180);
hold on;
plot(along_est.slope/pi*180);
grid on;
legend('Across Slope','Along Slope');
...
for i=1:nb_pings
    if ~isnan(true_idx_bottom(i))
        ...
        if across_est.delta(i) < phi_std_thr
            ...
            extended_across_angle(1:length(idx), i) = 0;
        end
...
new_echo_figure([]);
subplot(2,1,1)
imagesc(bs_bottom_uncomp,'CDataMapping','scaled');
...
xlabel('Ping Number');
ax3=subplot(1,3,3);
echo_sv=image(number,range,sv,'CDataMapping','scaled');
hold on;
% plot(number,bot_range,'r','linewidth',2);
...
if isempty(z_temp)
    ...
    choice=question_dialog_fig([],'','Do you want to continue with no SVP and constant velocity profile?');
    ...
    layer.EnvData.set_svp(z_temp,c_temp,'constant');
...
[x_uncorr,y_uncorr,z_uncorr]=arrayfun(@angles_to_pos_single,straight_range_bottom,zeros(size(straight_range_bottom)),zeros(size(straight_range_bottom)),...
...
new_echo_figure([]);
title('BS')
pcolor(lon_bot_ext,lat_bot_ext,bs_bottom);
hold on;
plot(long_ship(:),lat_ship(:),'b');
zlabel('Depth(m)')
set(gca,'fontsize',16)
colormap jet;
grid on;
...
if ~isequal(file_outputs,0)&&~isequal(path_out,0)
    ...
    ff=fieldnames(new_struct);
    
    for idi=1:numel(ff)
        if isrow(new_struct.(ff{idi}))
            new_struct.(ff{idi})=new_struct.(ff{idi})';
        end
    end
