...
for it=1:length(layers)
    ...
    if ~isempty(layers(it).SurveyData)
        ...
        for is=1:length(layers(it).SurveyData)
            ...
            output.Type{nb_survd}=survd.Type;
...
if ~isempty(output)
    output.SurveyName(cellfun(@isempty,output.SurveyName))={' '};
    output.Voyage(cellfun(@isempty,output.Voyage))={' '};
    output.Type(cellfun(@isempty,output.Type))={' '};
    output.Stratum(cellfun(@isempty,output.Stratum))={' '}; 
end
