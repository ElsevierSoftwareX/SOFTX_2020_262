...
if ~isempty(erased)
    ...
    for i=1:length(erased.channel(idx_chan))
        reg=create_reg_lsss_erased(erased.channel(idx_chan),'Bad Data','LSSS Erased',range,trans_obj.TransducerDepth);
...
for i=1:length(school)
    ...
    reg=create_reg_lsss(school(i),i,'Data','LSSS School',range,trans_obj.TransducerDepth);
    trans_obj.add_region(reg);
end
...
for i=1:length(layer)
    ...
    reg=create_reg_lsss(layer(i),i,'Data','LSSS Layer',range,trans_obj.TransducerDepth);
    trans_obj.add_region(reg);
end
...
if ~isempty(exclude)
    ...
    for i=1:length(exclude)
        [~, startPing] = min(abs(exclude(i).startTime - trans_obj.Time));
...
Idx_ping=nanmin(J):nanmax(J);
mask=mask(Idx_r,Idx_ping);
reg=region_cl(...
    'ID',1,...
    'Name',name,...
    'Tag','',...
