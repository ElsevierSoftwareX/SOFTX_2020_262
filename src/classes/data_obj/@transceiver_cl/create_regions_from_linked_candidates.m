...
idx_ping=nanmin(floor(p.Results.idx_ping)):nanmax(ceil(p.Results.idx_ping));
idx_r=nanmin(floor(p.Results.idx_r)):nanmax(ceil(p.Results.idx_r));

if ~all([numel(idx_r) numel(idx_ping)]==size(linked_candidates)) 
    x=repmat(p.Results.idx_ping,size(linked_candidates,1),1);
    F = scatteredInterpolant(x(p.Results.idx_r>0),p.Results.idx_r(p.Results.idx_r>0),linked_candidates(p.Results.idx_r>0),'nearest');
    [x_tot,y_tot]=meshgrid(idx_ping,idx_r);
...
for j=1:numel(classes)
    ...
    if ~isempty(J)
        ...
        idx_ping_sub=ping_ori:ping_ori+Bbox_w-1;
        ...
        if bbox_only==1
            reg_temp=region_cl(...
                ...
                'Type','Data',...
                'Idx_ping',idx_ping_sub+idx_ping(1)-1,...
            ...
            reg_temp=region_cl(...
                ...
                'Idx_r',idx_r_sub+idx_r(1)-1,...
                'Shape','Polygon',...
                'MaskReg',full(curr_reg(idx_r_sub,idx_ping_sub)),...
                'Reference',ref,...
                'Cell_h',cell_h,...
                'Cell_h_unit',h_unit,...
                'Cell_w',cell_w,...
