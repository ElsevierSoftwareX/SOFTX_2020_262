...
for ilay=1:length(layers_obj)
    ...
    if ~any(strcmp(p.UsingDefaults,'DbFile'))
        ...
        [~,file_lays_short] = layers_obj(ilay).get_path_files();
    end
    ...
    for ip=1:length(path_lay)
        try
            ...
            if ~(exist(db_file,'file')==2)
                initialize_echo_logbook_dbfile(path_f,p.Results.main_figure,0)
            end
            ...
            [files_rem,~]=setdiff(files_logbook,list_raw);
            
            for ifir=1:numel(files_rem)
               dbconn.exec(sprintf('delete from logbook where Filename is "%s"',files_rem{ifir})); 
            end
            
            [new_files,~]=setdiff(list_raw,files_logbook);
            new_files = setdiff(new_files,file_lays_short);
