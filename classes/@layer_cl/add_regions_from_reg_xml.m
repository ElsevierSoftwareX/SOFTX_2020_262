...
for ix=1:length(reg_file_str)
    ...
    for itrans=1:length(region_xml_tot)
        ...
        [trans_obj,~]=layer_obj.get_trans(region_xml.Infos);
        ...
        for i=1:length(reg_xml)
            ...
            if strcmpi(Cell_h_unit,'samples')
                Cell_h_unit='metrers';
                Cell_h=2;
                print_errors_and_warnings([],'warning','Samples slices in regions no longer supported, replaced by meters!');
            end
            
            
            switch ver
                case '0.1'
                    print_errors_and_warnings([],'warning','This region is imported from an old region file, if the soundspeed has been modified it night not be in the right place!');
