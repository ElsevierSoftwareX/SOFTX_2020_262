...
files_in_log=dbconn.fetch(sprintf('select Filename from logbook'));
...
[~,idx_sort]=sort(cellfun(@(x) datenum(x,'yyyy-mm-dd HH:MM:SS'),data_ori_new(:,10)));
data_ori_new=data_ori_new(idx_sort,:);
data_ori_new(:,12)=num2cell(1:size(data_ori_new,1));

[~,idx_rem]=setdiff(data_ori_new(:,2),files_in_log);
if ~isempty(idx_rem)
    data_ori_new(idx_rem,:)=[];
end
