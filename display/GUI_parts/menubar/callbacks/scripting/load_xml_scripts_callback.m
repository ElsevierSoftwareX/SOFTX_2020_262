...
%% Function
function load_xml_scripts_callback(~,~,main_figure)
...
if isempty(xml_surveys_input)
    load_scripts_fig(main_figure,[],'xml');
else
    ...
    load_scripts_fig(main_figure,xmlSummary,'xml');   
end
...
for i=idx_sort
    try
        ...
        disp_perso([],sprintf('Could not parse xml survey from file %s\n',Filenames(i).name));
