...
function [survey_data_out,modified]=edit_survey_data_fig(main_fig,survey_data_obj,editable,tt)
FigPos=[200 200 240 380];
modified=zeros(size(editable));
if isempty(tt)
    tt='Transect informations';
end
if isempty(survey_data_obj)
...
idx_t=find(strcmpi(deblank(survey_data_obj.Type),deblank(type_cell)));
if isempty(idx_t)
    idx_t=1;
end

uicontrol(edit_sd_fig,StInfo,'string','Survey name: ','pos',[10 320 90 20],'HorizontalAlignment','right');
survey_handle=uicontrol(edit_sd_fig,EdInfo,'pos',[100 320 100 20],'string',survey_data_obj.SurveyName,'enable',editable{1},'userdata',0,'callback',@up_userdata);

uicontrol(edit_sd_fig,StInfo,'string','Voyage: ','pos',[20 280 80 20],'HorizontalAlignment','right');
voy_handle=uicontrol(edit_sd_fig,EdInfo,'pos',[100 280 80 20],'string',survey_data_obj.Voyage,'enable',editable{2},'userdata',0,'callback',@up_userdata);

uicontrol(edit_sd_fig,StInfo,'string','Snapshot: ','pos',[20 240 80 20],'HorizontalAlignment','right');
snap_handle=uicontrol(edit_sd_fig,EdInfo,'pos',[100 240 40 20],'string',num2str(survey_data_obj.Snapshot),'callback',{@check_fmt_box,0,inf,0,'%.0f'},'enable',editable{3},'userdata',0);



uicontrol(edit_sd_fig,StInfo,'string','Type: ','pos',[20 200 80 20],'HorizontalAlignment','right');
type_handle=uicontrol(edit_sd_fig,LstboxInfo,'pos',[100 200 100 20],'string',type_cell,'value',idx_t,'enable',editable{4},'userdata',0,'callback',@up_userdata);

uicontrol(edit_sd_fig,StInfo,'string','Stratum: ','pos',[20 160 80 20],'HorizontalAlignment','right');
strat_handle=uicontrol(edit_sd_fig,EdInfo,'pos',[100 160 60 20],'string',survey_data_obj.Stratum,'enable',editable{5},'userdata',0,'callback',@up_userdata);

uicontrol(edit_sd_fig,StInfo,'string','Transect: ','pos',[20 120 80 20],'HorizontalAlignment','right');
trans_handle=uicontrol(edit_sd_fig,EdInfo,'pos',[100 120 40 20],'string',survey_data_obj.Transect,'callback',{@check_fmt_box,0,inf,0,'%.0f'},'enable',editable{6},'userdata',0);

uicontrol(edit_sd_fig,StInfo,'string','Comment: ','pos',[20 80 80 20],'HorizontalAlignment','right');
comment_handle=uicontrol(edit_sd_fig,EdInfo,'pos',[20 40 200 40],'string',survey_data_obj.Comment,'enable',editable{7},'Max',10,'Min',1,'userdata',0,'callback',@up_userdata);
...
% figure was deleted.
if ishghandle(edit_sd_fig)
    modified=[voy_handle.UserData survey_handle.UserData snap_handle.UserData type_handle.UserData strat_handle.UserData trans_handle.UserData comment_handle.UserData];
